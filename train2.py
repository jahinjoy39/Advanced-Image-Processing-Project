...
from niftiThing import get_datasets_nifti
...
def parse_args():
    ...
    parser.add_argument("--nifti_data_path", type=str, required=True,
    ...
    if not upd_config.eval:
        ...
        for _ in range(first_epoch, h_config.num_train_epochs):
            ...
            for batch_data in train_dataloader: # Renamed 'val_batch' to 'batch_data' for clarity
                ...
                # --- CUSTOM: EARLY NIFTI SAVING (Debugging) ---
                #if accelerator.is_main_process:
                    #save_dir = Path(h_config.output_dir) / "early_train_nifti_samples"
                    ...
                    #for i in range(corrupted_batch.shape[0]):
                        ...
                        # Extract data and affine matrix (convert PyTorch to NumPy)
                     #   img_data = corrupted_batch[i].cpu().numpy().squeeze()
                     #   target_data = target_batch[i].cpu().numpy().squeeze()
                     #   affine = affine_batch[i]
                    #
                    #    # Reshape 2D data back into 3D volume (1, H, W) for NIfTI saving
                    #    dummy_3d_input = np.zeros((1, *img_data.shape), dtype=img_data.dtype)
                    #    dummy_3d_input[0, :, :] = img_data
                    #
                    #    dummy_3d_target = np.zeros((1, *target_data.shape), dtype=target_data.dtype)
                    #    dummy_3d_target[0, :, :] = target_data

                    #    # Save the corrupted input image (Anomaly Added)
                    #    nifti_input = nib.Nifti1Image(dummy_3d_input, affine)
                    #    nib.save(nifti_input, save_dir / f"step_{global_step+1}_sample_{i}_corrupted.nii.gz")

                        # Save the target image (Original/Clean)
                    #    nifti_target = nib.Nifti1Image(dummy_3d_target, affine)
                    #    nib.save(nifti_target, save_dir / f"step_{global_step+1}_sample_{i}_target.nii.gz")
                # ---------------------------------------------------
