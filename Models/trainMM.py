...
from niftiMM import get_datasets_nifti
from UPD_study.models.ours.early_stopping import MovingAverageEarlyStopper
from UPD_study.models.ours.restoration_task1 import get_task_split, RestorationTask
...
from UPD_study.models.ours.restoration_task import OTHER_DSET_BLEND
...
def anom_inference(unet, scheduler, h_config, upd_config, accelerator, weight_dtype, input_imgs):
    ...
    # List to store intermediate denoised samples for visualization
    ...
    with torch.no_grad(), torch.autocast("cuda"):
        for t in timesteps:
            # 1. Prepare input for conditional/unconditional prediction
            ...
            # 3. Apply Guidance (if enabled)
            if h_config.validation_guidance > 0:
                noise_pred_uncond, noise_pred_cond = noise_pred.chunk(2)
                noise_pred = noise_pred_uncond + h_config.validation_guidance * \
                    (noise_pred_cond - noise_pred_uncond)

            # 4. Denoise and save the new sample (x_{t-1})
            noise = scheduler.step(noise_pred, t, noise).prev_sample

            # --- CUSTOM: Save the current intermediate sample ---
            if (t.item() % 50 == 0) or (t.item() == timesteps[-1]):
                sample_copy = noise.clone().detach()

                # Uncenter and clamp for visualization if needed
                if upd_config.center:
                    sample_copy = sample_copy / 2 + 0.5

                intermediate_samples.append(sample_copy.clamp(0, 1))
            # --------------------------------------------------

    # Noise is now the fully sampled image
    diffusion_output = noise

    # Anomaly map calculation (rest of original logic)
    ...
    # Zero out background (original logic)
    if upd_config.zero_bg_pred and upd_config.modality == 'MRI': # NOTE: Change to 'NIFTI' if needed
        # Assuming your NIFTI data should not use this MRI specific mask
        # If your background is truly zero, you can skip this block or adapt it.
        pass

    anomaly_score = anomaly_map.mean(dim=(1, 2, 3))

    # Uncenter images for visualisation
    if upd_config.center:
        diffusion_output = diffusion_output / 2 + 0.5
    restored_imgs = diffusion_output.clamp(0, 1)

    # --- CUSTOM: Return intermediate steps ---
    return anomaly_map, anomaly_score, restored_imgs, intermediate_samples
...
def log_validation(unet, scheduler, h_config, upd_config, accelerator, weight_dtype, val_dataset: BaseDataset):
    ...
    anom_maps, _, restored_imgs, _ = anom_inference(
    ...
    if images.shape[-1] == 2:
        # Average the two channels to get a single grayscale channel
        images = np.mean(images, axis=-1, keepdims=True)
        # Reshape to 3 channels by repeating the single channel (standard practice for color image logging)
        images = np.repeat(images, 3, axis=-1).astype(np.uint8)
...
def log_test_examples(unet, scheduler, h_config, upd_config, accelerator, weight_dtype, val_dataset: BaseDataset, indices=None):
    ...
    start = time.time()
    # B x C x H x W, intensity range should match that of conditional
    anom_maps, _, restored_imgs, intermediate_steps = anom_inference(
    ...
    if len(intermediate_steps) > 0:
        # Stack intermediate samples: B x T x C x H x W
        intermediate_tensors = torch.stack(intermediate_steps, dim=1) 
        
        # Take the first image's steps: 1 x T x C x H x W
        # We'll log the evolution of the first image in the batch
        first_img_evolution = intermediate_tensors[0, :, :, :, :] 
        
        # Arrange horizontally: C x H x (T * W)
        evolution_img = rearrange(first_img_evolution, 't c h w -> c h (t w)') 
        
        example_results[f'anom_test/denoising_evolution'] = evolution_img.cpu()
...
def parse_args():
    ...
    parser.add_argument("--nifti_data_path", type=str, default=".",
                        help="Path to the directory containing NIfTI files.")
    parser.add_argument("--t1_path", type=str, required=True,
                    help="Directory with T1/T1c NIfTI files")
    parser.add_argument("--t2_path", type=str, required=True,
                    help="Directory with T2/T2-FLAIR NIfTI files")
    
    #parser.add_argument("--img_channels", type=int, default=2,
    #                help="Number of MRI modalities (e.g., 2 for T1+T2)")
    parser.set_defaults(modality='NIFTI')
    
    upd_config = parser.parse_args()
    upd_config.img_channels = 2
...
def main():
    ...
    if not upd_config.eval:
        ...
        val_tasks = list(val_tasks)

        if not ext_blending_dsets: # This is how the original script should check this
            if OTHER_DSET_BLEND in train_tasks:
                train_tasks.remove(OTHER_DSET_BLEND)
        
        upd_config.aug_fn = RestorationTask(
            train_tasks, 
            has_bg=False, # Assuming NIFTI slices are pre-cropped, set to False
            center=upd_config.center,
            p_no_aug=upd_config.p_no_aug, 
            task_kwargs=h_config.task_kwargs,
            external_blending_dsets=ext_blending_dsets
        )
        val_task = RestorationTask(
            val_tasks, 
            has_bg=False, # Assuming NIFTI slices are pre-cropped, set to False
            center=upd_config.center,
            p_no_aug=upd_config.p_no_aug, 
        ...
        train_dataset.aug_fn = upd_config.aug_fn
        ...
        for _ in range(first_epoch, h_config.num_train_epochs):
            train_loss = 0.0
            NIFTI_SAVE_LIMIT = 2
            img = nib.load("/data/users1/afani1/children/labels_BOB/old/all/1m_sub-198549_babyHead-new3.nii.gz")
            affine = img.affine
            for batch_data in train_dataloader: # Renamed 'val_batch' to 'batch_data' for clarity

                # 1. Unpack the full batch (corrupted, target, affine)
                corrupted_batch, target_batch = batch_data

                # 2. Data needed for compute_loss: (corrupted, target)
                loss_batch = [corrupted_batch, target_batch]

                # --- CUSTOM: EARLY NIFTI SAVING (Debugging) ---
                #if accelerator.is_main_process:
                    #save_dir = Path(h_config.output_dir) / "early_train_nifti_samples_mm"
                    #save_dir.mkdir(parents=True, exist_ok=True)

                    #logger.info(f"Saving training NIfTI samples (Batch {global_step+1}) to {save_dir}")

                    # Iterate through the batch
                    #for i in range(corrupted_batch.shape[0]):
                        #batch_sample = corrupted_batch[i].cpu().numpy() # shape (C=2, H, W)
                        #target_sample = target_batch[i].cpu().numpy()   # shape (C=2, H, W)
    
    # 1. Access the fixed affine matrix
    # NOTE: You defined affine outside the loop using nib.load(). 
    # This is correct since you are reusing the affine from one file.
    
    # 2. Iterate over the two channels (Modalities T1 and T2)
                        #for chan in range(batch_sample.shape[0]):
                            #modality_name = f"mod{chan}" # Use 'mod0' for T1 and 'mod1' for T2

        # Extract single channel (H, W)
                            #img_data_2d = batch_sample[chan, :, :]
                            #target_data_2d = target_sample[chan, :, :]
        
        # Reshape the 2D data back into 3D volume (1, H, W) for NIfTI saving
        # The volume has a depth of 1, as it's a single slice.
                            #dummy_3d_input = img_data_2d[np.newaxis, :, :] # Shape: (1, H, W)
                            #dummy_3d_target = target_data_2d[np.newaxis, :, :] # Shape: (1, H, W)

        # Save the corrupted input (T1 and T2 saved separately)
                            #nifti_input = nib.Nifti1Image(dummy_3d_input, affine)
                            #nib.save(nifti_input, save_dir / f"step_{global_step+1}_sample_{i}_corrupted_{modality_name}.nii.gz")

        # Save the target image (Original/Clean)
                            #nifti_target = nib.Nifti1Image(dummy_3d_target, affine)
                           # nib.save(nifti_target, save_dir / f"step_{global_step+1}_sample_{i}_target_{modality_name}.nii.gz")                
                    # ---------------------------------------------------
                ...
                with accelerator.accumulate(unet):
                    ...
                    loss = compute_loss(
                        h_config, noise_scheduler, unet, uncond_p, loss_batch)
...
def compute_loss(h_config, noise_scheduler, unet, uncond_p, batch):
    ...
    print("MODEL INPUT SHAPE:", model_input.shape, flush=True)
    print("TIMESTEPS SHAPE:", timesteps.shape, flush=True)
