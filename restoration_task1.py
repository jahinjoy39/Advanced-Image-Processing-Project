...
from UPD_study.models.ours.task_utils.base_task1 import BaseTask
...
from UPD_study.models.ours.task_utils.patch_blending_task1 import PoissonImageEditingMixedGradBlender, \
...
class RestorationTask:
    ...
    def apply(self, img: np.ndarray, dset: BaseDataset) -> tuple[np.ndarray, np.ndarray]:
        ...
        result = self.task_map[self.rng.choice(self.task_ids)](img, dset)

        corrupted_img, original_img = result
        
        if isinstance(corrupted_img, torch.Tensor):
            corrupted_img = corrupted_img.cpu().numpy()
        ...
        return corrupted_img.astype(np.float32), original_img.astype(np.float32)
