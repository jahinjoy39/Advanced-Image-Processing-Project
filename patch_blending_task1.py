...
class BasePatchBlendingTask(BaseTask):
    ...
    def augment_sample(self, sample: np.ndarray[float], sample_mask: Optional[np.ndarray[bool]],
        ...
        #source_to_blend = np.clip(source_to_blend, torch.amin(source_sample, dim=spatial_axis, keepdim=True),
    #torch.amax(source_sample, dim=spatial_axis, keepdim=True))
        min_vals = np.min(source_sample, axis=spatial_axis, keepdims=True)
        max_vals = np.max(source_sample, axis=spatial_axis, keepdims=True)
        source_to_blend = np.clip(source_to_blend, min_vals, max_vals)
