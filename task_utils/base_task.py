...
class BaseTask(ABC):
    ...
    def apply(self, sample: np.ndarray[float],
        ...
        if isinstance(sample, np.ndarray):
            ...
            # This is the correct PyTorch method to clone a tensor
            aug_sample = sample.clone().detach() 
        else:
        ...
        #channel_min_vals = torch.amin(sample, dim=spatial_dims, keepdim=True)
        #channel_max_vals = torch.amax(sample, dim=spatial_dims, keepdim=True)
